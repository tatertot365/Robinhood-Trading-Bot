...
def calcAvg(SymbolsList):
    ...
    stockListHigh = r.stocks.get_stock_historicals(SymbolsList, interval='week', span='year', bounds='regular', info='high_price')
    stockListLow = r.stocks.get_stock_historicals(SymbolsList, interval='week', span='year', bounds='regular', info='low_price')
...
def buyStocks (buyTOrF, symbolsList):
    ...
    for i in range(len(buyTOrF)):
        if checkPositions() < 1000.00 and buyTOrF[i] == True:
            ...
            shareAmount = 10 / float(r.stocks.get_latest_price(symbolsList[i], priceType=None, includeExtendedHours=True)[0])
    ...
    for i in range(len(SymbolsList)):
        ...
        if SymbolsList[i] in r.account.build_holdings(with_dividends=False):
            ...
            if (avgBuyPrice < avgBidPrice):
                ...
                print('sold ' + str(quantity) + " shares " + str(SymbolsList[i]) + " with avg buy price of " + str(avgBuyPrice) + " and an avg bid price of " + str(avgBidPrice))
            ...
            #If there is a 15% difference between the average bid price and the average buy price a market order is places for the entire user owned quantity of the stock to stop losses
                ...
                #r.orders.order_sell_fractional_by_quantity(SymbolsList[i], quantity, timeInForce='gfd', extendedHours=False, jsonify=True)
                print('sell to stop loss ' + str(SymbolsList[i]) + " number of shares sold: " + str(quantity) + " with avg buy price of " + str(avgBuyPrice) + " and avg ask price of " + str(avgAskPrice))
